...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new meowcoincore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new meowcoincore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = meowcoincore.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new meowcoincore.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function meowcoincoreTest() {
      block1 = meowcoincore.Block.fromString(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('meowcoincore', meowcoincoreTest, {maxTime: maxTime});
