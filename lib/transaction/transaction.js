...
/**
 * Retrieve a hexa string that can be used with meowcoind's CLI interface
 ...
 *
 * Can receive, as output information, the output of meowcoind's `listunspent` command,
 * and a slightly fancier format recognized by meowcoincore:
 *
 * ```
 * {
 ...
 * }
 * ```
 * Where `address` can be either a string or a meowcoincore Address object. The
 * same is true for `script`, which can be a string or a meowcoincore Script.
 *
 ...
 *
 * // From a pay to public key hash output from meowcoind's listunspent
...
/**
 * Analogous to meowcoind's IsCoinBase function in transaction.h
 */
