...
function Transaction(serialized) {
  ...
  this._changeAssetScripts = {};
  this._changeAssetIndexes = {};
...
Transaction.prototype._hasDustOutputs = function(opts) {
  ...
  for (index in this.outputs) {
    ...
    if (output.satoshis < Transaction.DUST_AMOUNT && !(output.script.isDataOut() || output.script.isAssetOut())) {
...
Transaction.prototype.toObject = Transaction.prototype.toJSON = function toObject() {
  ...
  if (!_.isUndefined(this._changeAssetScripts)) {
    obj.changeAssetScripts = _.map(this._changeAssetScripts, function(script) { 
      return script.toString()
    });
  }
  if (!_.isUndefined(this._changeAssetIndexes)) {
    obj.changeAssetIndexes = this._changeAssetIndexes;
  }
...
Transaction.prototype.fromObject = function fromObject(arg) {
  ...
  if (transaction.changeAssetIndexes) {
    this._changeAssetIndexes = transaction.changeAssetIndexes;
  }
  if (transaction.changeAssetScripts) {
    this._changeAssetScripts = transaction.changeAssetScripts;
  }
  if (transaction.fee) {
 ...
 * @return {Transaction} this, for chaining
 */
Transaction.prototype.change = function(address, asset) {
  $.checkArgument(address, 'address is required');
  if (_.isUndefined(asset)) {
    this._changeScript = Script.fromAddress(address);
  } else {
    this._changeAssetScripts[asset] = Script.buildPublicKeyHashOut(address);
  }
  this._updateChangeOutput();
  return this;
};
 ...
 * @return {Transaction} this, for chaining
 */
Transaction.prototype.to = function(address, amount, asset) {
  if (_.isArray(address)) {
    var self = this;
    _.each(address, function(to) {
      self.to(to.address, to.satoshis, to.asset);
    });
    return this;
  }
  ...
  var satoshis;
  if (_.isUndefined(asset)) {
    script = Script(addressObj)
    satoshis = amount;
  } else {
    var assetObj = new Asset({ type: Asset.assetTypes.TRANSFER, name: asset, amount: amount });
    script = Script.buildPublicKeyHashOut(address).assetOut(assetObj);
    satoshis = 0;
  }
...
Transaction.prototype._getInputAmount = function() {
  if (_.isUndefined(this._inputAmount)) {
    ...
    _.each(this.inputs, function(input) {
      ...
      if (!input.output.script.isAssetOut()) {
...
Transaction.prototype._updateChangeOutput = function() {
  ...
  _.each(this._changeAssetIndexes, function(index) {
    this._removeOutput(index);
  });
  this._changeAssetIndexes = {};
  var availableAssets = this._getUnspentAssetValues();
  _.each(availableAssets, function(amount, asset) {
    if (amount > 0) {
      if (!_.isUndefined(self._changeAssetScripts[asset])) {
        self._changeAssetIndexes[asset] = self.outputs.length;
        var assetObj = new Asset({
          type: Asset.assetTypes.TRANSFER,
          name: asset,
          amount: amount
        });
        self._changeAssetScripts[asset] = self._changeAssetScripts[asset].assetOut(assetObj);
        self._addOutput(new Output({
          script: self._changeAssetScripts[asset],
          satoshis: 0
        }));
      }
    }
...
Transaction.prototype._getUnspentAssetValues = function () {
  var inputs = this._getInputAssetAmounts();
  var outputs = this._getOutputAssetAmounts();
  var keyset = _.intersection(_.keys(inputs), _.keys(outputs));
  var values = {};
  _.each(keyset, function(key) {
    values[key] = (inputs[key] || 0) - (outputs[key] || 0);
  });
  return values;
};

Transaction.prototype._getInputAssetAmounts = function () {
  var amounts = {};
  _.each(this.inputs, function(input) {
    if (_.isUndefined(input.output))
      throw new errors.Transaction.Input.MissingPreviousOutput();
    if (input.output.script.isAssetOut()) {
      var asset = input.output.script.getData();
      if (!_.has(amounts, asset.name)) {
        amounts[asset.name] = 0;
      }
...
Transaction.prototype._getOutputAssetAmounts = function () {
  var amounts = {};
  _.each(this.outputs, function(output) {
    if (output.script.isAssetOut()) {
      var asset = output.script.getData();
      if (!_.has(amounts, asset.name)) {
        amounts[asset.name] = 0;
      }
      amounts[asset.name] += asset.amount;
